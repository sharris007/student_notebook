  ...
  init = config => {
    ...
    const App = () => (
      <IntlProvider locale={locale}>
        <MuiThemeProvider>
          <ComponentOwner notesList={config.notesList} originalNotesList={config.originalNotesList} tocData={config.tocData} lastUsedFilters={config.lastUsedFilters} toolbarMode={config.toolbarMode} tagAttributes={config.tagAttributes} groupModeFlag={config.groupModeFlag} callback={config.callback} handleGroupClick={config.handleGroupClick}/>
        </MuiThemeProvider>
      </IntlProvider>
    );
    ReactDOM.render(
